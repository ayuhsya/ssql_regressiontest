...
import sstest.ExecRegressionTest.SSQL_exec;
import sstest.GUI.TestResult_Detail.LineNumberView;
...
//							}
							...
							if (SSQL_exec.execSuperSQL(ssqlQueryName2,classPath, driver, db, null, null,resultDirTmp)) {
								...
								if(!Dialog.YesNoDialog("Resist OK?")){
									...
									//								DBに登録する部分(insert())
									if(insert(seq_num))
										sstest.Common.Dialog.NormalDialog("Succeed(queryAdd)");
									else
										sstest.Common.Dialog.NormalDialog("Failed(queryAdd)");	
									dispose();
								}
